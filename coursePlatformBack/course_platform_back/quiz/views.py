...
def submit_test(request, video_id):
    try:
        ...
        for question_id, selected_answer in data['answers'].items():
            question = get_object_or_404(Question, id=question_id)  # Получаем вопрос по ID
            correct_answer = question.answer.correct_option  # Получаем правильный ответ
            if int(selected_answer) == correct_answer:  # Проверяем, совпадает ли выбранный ответ с правильным
                score += 1
